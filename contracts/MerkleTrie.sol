...
library MerkleTrie {
    ...
    function verifyInclusionProof(bytes memory _key, bytes memory _value, bytes memory _proof,
                                  bytes32 _root)internal pure returns (bool){
     ...
     */
    function verifyExclusionProof(bytes memory _key, bytes memory _value, bytes memory _proof, 
                                  bytes32 _root)internal pure returns (bool){
     ...
     */
    function get(bytes memory _key, bytes memory _proof, bytes32 _root)internal pure returns
                (bool, bytes memory){
        TrieNode[] memory proof = parseProof(_proof);
        (uint256 pathLength, bytes memory keyRemainder, ) = walkNodePath(proof, _key, _root);
     ...
     */
    function verifyProof(bytes memory _key, bytes memory _value, bytes memory _proof, bytes32 _root,
                         bool _inclusion)private pure returns (bool){
        TrieNode[] memory proof = parseProof(_proof);
...
// Otherwise, we need to make sure that we've hit a dead end.
     ...
     * )
     */
    function walkNodePath(TrieNode[] memory _proof, bytes memory _key, bytes32 _root)private pure returns(
                          uint256, bytes memory, bool)
    {
...
// If our node ID is NULL, then we're at a dead end.
     ...
     */
    function getNewPath(TrieNode[] memory _path, uint256 _pathLength, bytes memory _keyRemainder,
                        bytes memory _value) private pure returns (TrieNode[] memory){
...
// Since we're overwriting the last node in the path, we use `_pathLength - 1`.
     ...
     */
    function getUpdatedTrieRoot(TrieNode[] memory _nodes, bytes memory _key) private pure returns (bytes32){
        bytes memory key = BytesLib.toNibbles(_key);
...
// Simply return the hash of its encoding.
     ...
     */
    function parseProof(bytes memory _proof)private pure returns (TrieNode[] memory){
     ...
     */
    function getNodeID(RLPReader.RLPItem memory _node)private pure returns (bytes32){
        bytes memory nodeID;
        if (_node.len < 32) {
...
// Nodes 32 bytes or larger are hashed.
     ...
     */
    function getNodePath(TrieNode memory _node) private pure returns (bytes memory){
     ...
     */
    function getNodeKey(TrieNode memory _node)private pure returns (bytes memory){
     ...
     */
    function getNodeValue(TrieNode memory _node) private pure returns (bytes memory){
     ...
     */
    function getNodeHash(bytes memory _encoded)private pure returns (bytes memory){
     ...
     */
    function getNodeType(TrieNode memory _node)private pure returns (NodeType){
     ...
     */
    function getSharedNibbleLength(bytes memory _a, bytes memory _b) private pure returns (uint256){
        uint256 i = 0;
     ...
     */
    function makeNode(bytes[] memory _raw)private pure returns (TrieNode memory){
     ...
     * @return Node as a TrieNode struct.
     */
    function makeNode(RLPReader.RLPItem[] memory _items)private pure returns (TrieNode memory){
     ...
     */
    function makeExtensionNode(bytes memory _key, bytes memory _value)private pure 
     ...
     */
    function makeLeafNode(bytes memory _key, bytes memory _value)private pure returns (TrieNode memory){
        bytes[] memory raw = new bytes[](2);
     ...
     */
    function makeEmptyBranchNode()private pure returns (TrieNode memory){
     ...
     */
    function editBranchValue(TrieNode memory _branch, bytes memory _value) private pure 
                             returns (TrieNode memory){
     ...
     * @return Modified branch node.
     */
    function editBranchIndex(TrieNode memory _branch, uint8 _index, bytes memory _value) private pure
                             returns (TrieNode memory){
     ...
     * @return Prefixed key.
     */
    function addHexPrefix( bytes memory _key, bool _isLeaf) private pure returns (bytes memory){
     ...
     */
    function removeHexPrefix(bytes memory _path) private pure returns (bytes memory){
     ...
     */
    function joinNodeArrays(TrieNode[] memory _a, uint256 _aLength, TrieNode[] memory _b, uint256 _bLength)
